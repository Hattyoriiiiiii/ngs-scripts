...
# summaryディレクトリへのパス
alignRes <- paste0(projPath, "/summary/bowtie2/")
dupRes <- paste0(projPath, "/summary/picard/")

# sampleのprefix
sampleList <- alignRes %>% list.files() %>% lapply(function(x) strsplit(x, "_bowtie2.txt")[[1]][1]) %>% unlist()
print(sampleList)
...
for (sample in sampleList) {
    alignRes <- read.table(paste0(projPath, "/summary/bowtie2/", sample, "_bowtie2.txt"), header=FALSE, fill=TRUE)
